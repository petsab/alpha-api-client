...
class AvailCableCarResponseMapperTest extends TestCase
{
    ...
    /**
     * @dataProvider getCreateSourcingCarFromResponseUnavailableCarInput
     * @param string $input
     * @throws \Exception
     */
    public function testCreateSourcingCarFromResponseUnavailableCar(string $input)
    {
        $response = json_decode($input, true);
        $mapper = new CarResponseMapper($this->carDOFactory, $this->urlResponseMapper);
        $car = $mapper->map($response);
        ...
        $this->assertSame($response['thumbnail_url']['full'], $car->getThumbnailUrl()->getFull());
        $this->assertSame($response['available_car'], $car->isAvailable());
        $this->assertSame([], $car->getPremiumFeatures());
        $this->assertNull($car->getTurnover());
        $this->assertNull($car->getMeasure());
    }
    ...
    {
        $path = __DIR__ . DIRECTORY_SEPARATOR . "input" . DIRECTORY_SEPARATOR . "testCreateCarAFromResponse.php";
     ...
     * @return array
     */
    public function getCreateSourcingCarFromResponseUnavailableCarInput()
    {
        $path = __DIR__ . DIRECTORY_SEPARATOR . "input" . DIRECTORY_SEPARATOR . "testCreateCarUFromResponse.php";
